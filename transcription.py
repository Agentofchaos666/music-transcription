...
def preprocess_wav_file(filename):
    ...
    paddedX = np.zeros((CQT_result.shape[0], CQT_result.shape[1] + WINDOW_SIZE - 1), dtype=complex)
    pad_amount = WINDOW_SIZE / 2
    paddedX[:, pad_amount:-pad_amount] = CQT_result
    frame_windows = np.asarray([paddedX[:, i:i+WINDOW_SIZE] for i in range(CQT_result.shape[1])])
...
def get_wav_midi_data(filenames):
    ...
    for wav_file, midi_file in filenames:
        X_i = preprocess_wav_file(wav_file)
        Y_i = preprocess_midi_truth(midi_file)
        numSlices = min(X_i.shape[0], Y_i.shape[1])
        X_list.append(X_i[:numSlices])
...
=======
        print '===================================='
        print 'EPOCH [', epoch, ']'
>>>>>>> fa1177b5008f9f7ab49a485e23bed9a47bd8abed
